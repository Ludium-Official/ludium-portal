...
export const carouselItemsColumns: ColumnDef<CarouselItem>[] = [
...

  {
    ...
    cell: ({ row }) => {
      ...
      if (data.__typename === "Post") {
        return <Link target="_blank" to={`/community/posts/${row.original.itemId}`} className="underline">Click</Link>
      }
      if (data.__typename === "Program") {
        return <Link target="_blank" to={`/programs/${row.original.itemId}`} className="underline">Click</Link>
      }
    }
  },
...
const DeleteItemButton = ({ id }: { id: string }) => {
  ...
  return (
    <Button variant="destructive" onClick={() => deleteCarouselItem({ variables: { id: id } }).then(() => client.refetchQueries({ include: [CarouselItemsDocument] }))}>
      Delete
    </Button>
  )
}