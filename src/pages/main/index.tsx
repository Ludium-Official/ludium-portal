    ...
    <div className="mx-10 my-[60px]">
      <section className="flex justify-between items-center mb-20">
        <div>
          ...
          <Button type="button" variant="purple" className="w-[152px] h-11" >VIEW DETAIL</Button>
        </div>
          ...
          {data?.programs?.data?.map((program) => (
            <div key={program.id} className="border min-w-[425px] p-6 rounded-lg">
              ...
              <div className="flex items-center gap-4 mb-4">
                ...
                <Link to={`/programs/${program.id}`} className="text-lg font-bold truncate max-w-[310px]">
                  {program?.name}
                </Link>
              </div>
              <div className="mb-4">
              ...
              <div className="space-x-3 mb-2">
                <Link to={`/programs/${program.id}#applications`} className="text-xs font-semibold bg-[#F4F4F5] rounded-full px-2.5 py-0.5 leading-4">
                  Submitted Application <span className="text-[#B331FF]">{program.applications?.length ?? 0}</span>
                </Link>
                <Link to={`/programs/${program.id}#applications`} className="text-xs font-semibold bg-[#18181B] text-white rounded-full px-2.5 py-0.5">
