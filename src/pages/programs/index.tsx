...
const ProgramsPage: React.FC = () => {
    ...
    },
    {
      ...
      render: (program) => (
        <span className="text-xs">
          {program.network &&
            getNetworkDisplayName(program.network.chainName || "educhain")}
        </span>
      ),
    },
    {
  ...
  return (
    <Container
      className={cn(
        "bg-white rounded-2xl p-10 max-w-full",
        isMobile && "p-4 rounded-none"
      )}
    >
      <div className="flex justify-between items-center pb-4">
        <h1 className={cn("text-3xl font-bold", isMobile && "text-lg")}>
          Recruitment
        </h1>
        {isMobile && isLoggedIn && (
          <Button
      ...
      <Tabs value={selectedTab} onValueChange={setSelectedTab}>
        ...
        >
          <div></div>
          <div
            className={cn("flex items-center gap-3 h-10", isMobile && "w-full")}
          >
            <Input
          ...
          {loading ? (
            ...
            isMobile ? (
              <MobileCardList
                ...
                fields={mobileFields}
                onItemClick={(program) =>
                  navigate(`/programs/recruitment/${program.id}`)
                }
