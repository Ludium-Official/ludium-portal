...
import type { MilestoneType } from '@/pages/programs/details/_components/create-application-form';
...
export interface ApplicationDynamicTabsProps {
  ...
  onTabsChange: (tabs: ApplicationTabItem[]) => void;
...
export function ApplicationDynamicTabs({
  ...
  const handleAddTab = () => {
    ...
    const newTab: ApplicationTabItem = {
      id: newTabId,
      label: `Milestone ${dynamicTabs.length + 1}`,
...
export function useApplicationDynamicTabs(initialTabs: ApplicationTabItem[] = []) {
  const [tabs, setTabs] = React.useState<ApplicationTabItem[]>(initialTabs);

  // Add a new tab
  const addTab = React.useCallback((tab: Omit<ApplicationTabItem, 'id'>) => {
    const newTab: ApplicationTabItem = {
      ...tab,
  ...
  // Update a tab by id
  const updateTab = React.useCallback((tabId: string, updates: Partial<ApplicationTabItem>) => {
