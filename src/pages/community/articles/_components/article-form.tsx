...
import { ArticleStatus, ArticleType } from '@/types/types.generated';
    ...
    <>
      {isMobile && (
        <MobileFormHeader
          ...
          backLink={isEdit ? `/community/articles/${id}` : '/community/articles'}
          isPublished={data?.article?.status === ArticleStatus.Published}
          loading={loading}
          onPublish={handleSubmit((formData) => onSubmit(formData, 'publish'))}
      ...
      <Container className={cn('px-25 py-[30px]', isMobile && 'px-4 py-7')}>
        <form className="w-full mx-auto">
          {!isMobile && (
            <div
              className={cn(
                'flex items-center w-fit mb-17 text-sm text-muted-foreground',
                isMobile && 'mb-6',
              )}
            >
          ...
          <section className={cn('px-12 mb-2', isMobile && 'px-0')}>
            {isAdmin && (
              <div
                className={cn(
                  'flex items-end gap-6 mb-8',
                  isMobile && 'flex-col items-start gap-4 mb-10',
                )}
              >
                <InputLabel labelId="category" title="Category" inputClassName="hidden" isPrimary>
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button
                        variant="outline"
                        className={cn('w-[400px] justify-between h-10', isMobile && 'w-full')}
                      >
                    ...
                    <DropdownMenuContent
                      className={cn('w-[400px]', isMobile && 'w-full min-w-[343px]')}
                    >
            ...
            <label htmlFor="image" className="space-y-2">
              <div className="flex items-start gap-6 mb-8">
                ...
                <div className="flex-1">
                  <p className={cn('font-medium text-base', isMobile && 'text-sm')}>
                    Cover image<span className="ml-[2px] text-red-500">*</span>
                  </p>
                  <p className={cn('text-sm text-muted-foreground', isMobile && 'mt-2 text-xs')}>
