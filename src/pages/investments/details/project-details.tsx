...
function ProjectDetailsPage() {
  ...
  useEffect(() => {
    if (program?.fundingCondition === "tier" && program?.userTierAssignment) {
      ...
      const maxAmountFromSettings =
        tierSettings?.[tier as keyof typeof tierSettings]?.maxAmount;
      const maxAmountFromAssignment =
  ...
  const handleInvestThroughFiatonramp = async () => {
    ...
    try {
      ...
      if (program?.fundingCondition === "tier") {
        ...
        rawAmount =
          userTierAssignment?.maxInvestmentAmount?.toString() ?? undefined;
      } else {
      ...
      try {
        ...
        popup = window.open(
          paymentUrl,
          "swapped-payment",
          "width=445,height=585,scrollbars=yes"
        );
        ...
        const statusCheckInterval = setInterval(async () => {
          try {
            const statusResponse = await fetch(
              `/api/swapped/order-status/${orderId}`
            );
            ...
            if (statusResponse.ok) {
              ...
              if (status === "order_completed") {
                ...
                notify(
                  "Payment completed! Processing investment...",
                  "success"
                );
                await handleInvest();
        ...
        setTimeout(() => {
          ...
          if (isInvesting) {
            notify(
              "Payment timeout. Please check your transaction status.",
              "error"
            );
            setIsInvesting(false);
          }
        ...
        console.error(
          "Backend order creation failed, trying direct widget:",
          orderError
        );
      ...
      const messageHandler = async (event: MessageEvent) => {
        ...
        if (event.data.type === "SWAPPED_ORDER_COMPLETED") {
          ...
          try {
            ...
            notify(
              "Payment completed but investment failed. Please contact support.",
              "error"
            );
          }
      ...
      notify(
        (error as Error).message || "Failed to open payment widget",
        "error"
      );
      setIsInvesting(false);
    }
            ...
            if (
              ...
              const isProgramOwner =
                ...
                currentUserAddress.toLowerCase() ===
                  programOwnerAddress.toLowerCase();
      ...
      try {
        ...
        console.log("Step 6 - Saving investment to database:", {
          ...
          selectedTerm: selectedTerm
            ? { id: selectedTerm?.id, price: selectedTerm?.price }
            : null,
