...
const CONTRACT_ADDRESSES: Record<string, InvestmentContractAddresses> = {
  ...
  creditcoin: {
    ...
    milestone:
      import.meta.env.VITE_CREDITCOIN_INVESTMENT_MILESTONE_ADDRESS || "",
...
export function useInvestmentContract(network: string | null = null) {
  ...
  if (isExternalWallet && activeWallet) {
    console.log(
      "Using external wallet for investment transactions:",
    ...
    sendTx = async (input, _uiOptions?: unknown) => {
      ...
      try {
        console.log(
          "Sending investment transaction with external wallet...",
          input
        );
        ...
        console.error(
          "Error sending investment transaction with external wallet:",
          error
        );
        throw error;
      }
  ...
  const contractAddresses =
    CONTRACT_ADDRESSES[network || "educhain-testnet"] ||
    CONTRACT_ADDRESSES["educhain-testnet"];
